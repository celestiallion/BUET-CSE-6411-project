...
\end{center}
\end{table}
Now we oversample the classes in the dataset randomly so that each class has the same number of samples. We did not subsample any class to be as inclusive of patterns as possible so that the classification accuracy remains high.
\section{Methodology}
...
We extend two sequence embedding techniques to the domain of biological sequences- word2vec and GloVe. However, rather directly using them, we modify them for using with k-mers. While using word2vec for generating an embedding, we used a neural network as an autoencoder. However, we could not have the training procedure converged. Later we went for a variant of GloVe algorithm where we picked only the static part of generating the histogram. It worked fine.\\
\par 
As k-mers we pick 3-mers, that is the codons. We did not pick the value of $k$ experimentally. We picked a value of $k$ that corresponds to a already known and studied biological structure.\\
\par
...
\par 
The decisions trees that are grown very deep tend so that very irregular patterns can be learned. Therefore, they have low bias, but very high variance that is overfit their training sets. To reduce the variance Random forests grow multiple deep decision trees with each trees trained on different random parts of the same training set. Although this improves the accuracy but it becomes more difficult to interpret the results.\\
\par 
We used $5$ decision trees in our Random Forest classifier. Our splitting ciriterion was gini impurity index. The max depth is not defineed. The nodes are expanded until all leaves are pure. When looking for the best split we looked at $64$ features among the $4096$ features. Samples were bootstrapped during growing the decision trees. As we already balanced the samples per different classes we did not have to consider issues regarding imablanced data. 
...
\includepdf{../src/visualizations/estimator_0.pdf}
\includepdf{../src/visualizations/estimator_1.pdf}
\includepdf{../src/visualizations/estimator_2.pdf}
\includepdf{../src/visualizations/estimator_3.pdf}
\includepdf{../src/visualizations/estimator_4.pdf}
